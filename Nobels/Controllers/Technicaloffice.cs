...
namespace Nobels.Controllers
{
    ...
    //roleId:13
    [Authorize]
    public class Technicaloffice : Controller
    {
        ...
        public Technicaloffice(db_a8d242_exadecor2Context context, UserManager<Employee> userManager, IPasswordHasher<Employee> passwordHasher)
        {
            _context = context;
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    qu.QuotationStatus = "مقبول فنياً";
                    _context.Update(qu);
                    await _context.SaveChangesAsync();
                    ...
                    update.EmployeeId = Int32.Parse(userId);
                    //var employee = _context.Employees.FirstOrDefault(x => x.Id == update.EmployeeId);
                    update.Updates = "تم قبول العرض فنياً بتاريخ" + DateTime.Now;
                    _context.Add(update);
        ...
        public async Task<IActionResult> refuse(int id)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    qu.QuotationStatus = "مرفوض فنياً";
                    _context.Update(qu);
                    await _context.SaveChangesAsync();
                    ...
                    update.EmployeeId = Int32.Parse(userId);
                    //var employee = _context.Employees.FirstOrDefault(x => x.Id == update.EmployeeId);
                    update.Updates = "تم رفض فنياًالعرض بتاريخ" + DateTime.Now;
                    _context.Add(update);
        ...
        public async Task<IActionResult> DeleteConfirmed(int id ,string note)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (note != null || note != String.Empty) {  QoutationUpdate update = new QoutationUpdate();
                    var userId = HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
                    update.ChangeDate = DateTime.Now;
                    update.QoutationId = qu.QuotationId;
                    update.EmployeeId = Int32.Parse(userId);
                    //var employee = _context.Employees.FirstOrDefault(x => x.Id == update.EmployeeId);
                    update.Updates = "تم رفض العرض فنياً بتاريخ" + DateTime.Now + " ," + "السبب:" + note;
                    _context.Add(update);
        ...
        public IActionResult Index()
        {
            ...
            //from bill in _context.Bills
               ...
               where (/*qu.QuotationStatus == "جديد"||*/qu.QuotationStatus== "قيد التدقيق الفني")&& _context.OrderDetails.Where(o => o.QoutationId == qu.QuotationId).Any()
        ...
        public IActionResult RefuseQoutation()
        {
            ...
            //from bill in _context.Bills
               ...
               where qu.QuotationStatus == "مرفوض فنياً"
            ...
            var qUpdate = _context.QoutationUpdates.Where(q=>q.Updates.Contains("تم رفض العرض فنياً")).ToList();
        ...
        public IActionResult AcceptQoutation()
        {
            ...
            //from bill in _context.Bills
               ...
               join add in _context.Addresses on qu.addressId equals add.id
               join city in _context.Cities on add.cityid equals city.CityId
               where _context.QoutationUpdates.Where(i=>i.QoutationId==qu.QuotationId&&i.Updates.Contains("تم قبول العرض فنياً")).Any()&& qu.QuotationStatus != "مرفوض فنياً"
                        ...
                        BillModel
               {
                   ...
                   branch = branch.BranchName,
                   BillDate = _context.QoutationUpdates.Where(i => i.QoutationId == qu.QuotationId && i.Updates.Contains("قبول العرض فنياً")).First().ChangeDate,
                   quationId = qu.QuotationId,
                   employeeName = Employee.FirstName + Employee.LastName,
                   status = qu.QuotationStatus,
                   code = qu.Code,
        ...
        public async Task<IActionResult> deleteOrderConfirmed(int id)
        {
            ...
            if (orderDetail != null)
            {
                ...
                update.EmployeeId = Int32.Parse(userId);
                var employee = _context.Employees.FirstOrDefault(x => x.Id == update.EmployeeId);
                update.Updates = "تم الحذف بتاريخ:" + " " + DateTime.Now + " " + "من قبل   " + " " + " المكتب الفني"
                   ;
            ...
            await _context.SaveChangesAsync();
            return RedirectToAction("OrderDetailsUpdate", "Technicaloffice", new { id = orderDetail.QoutationId });
        }
